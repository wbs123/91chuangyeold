...
class TopController extends HomeBaseController
{
    ...
    {
        $lick1 = db("portal_xm")->where('typeid1 = 2')->order('pubdate asc')->limit(16)->select();
        ...
        $cate = db("portal_category")->where('id', 'in', $arr)->order('list_order asc')->limit(15)->select();
        $cates = db("portal_category")->where('id', 'in', $arr)->order('list_order asc')->select();
        foreach($cates as $key=>$val)
        {
            $val['data'] = db("portal_xm")->where('typeid1 = '.$val['id'])->order('click asc')->limit(10)->select();
            $data[] = $val;
        }
    ...
    public function list_top()
    {
        ...
        $tdk = db('portal_category')->where("id = $id")->find();
        ...
        {
            $val['data'] = db("portal_xm")->where('typeid1 = '.$val['id'])->order('pubdate asc')->limit(10)->select();
            $data[] = $val;
        }
    ...
    public function index_top()
    {
        ...
        $cate = db("portal_category")->where('id', 'in', $arr)->order('list_order asc')->limit(15)->select();
        $name = db("portal_category")->where("id = ".$id)->value('name');
        $lick1 = db("portal_xm")->where('typeid1 = '.$id)->order('click asc')->limit(10)->select();
        $lick2 = db("portal_xm")->where('typeid1 = '.$id)->order('click asc')->limit(0,8)->select();
        $lick3 = db("portal_xm")->where('typeid1 = '.$id)->order('click asc')->limit(8,8)->select();
        $lick4 = db("portal_xm")->where('typeid1 = '.$id)->order('click asc')->limit(16,8)->select();
        $lick5 = db("portal_xm")->where('typeid1 = '.$id)->order('click asc')->limit(24,8)->select();
