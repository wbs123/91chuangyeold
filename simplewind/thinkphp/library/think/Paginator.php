...
//        }
//        var_dump($arr);
//        $path1 = preg_replace('/\/list_(\d+).html/i','',$arr[0]);
//        var_dump($path1);exit;

        $path = explode('/',$path);
        $keys = 3;
        if(isset($path[$keys])){
            $pathq = explode('.',$path[$keys]);
            if(in_array("html",$pathq)){
                unset($pathq[1]);
                $paths = array_merge($pathq);
                $patha = implode($paths);
                $path[$keys] = $patha;
            }
        ...
        foreach($pathArr as $key=>$values ){
            if (strstr( $values , 'list_' ) !== false ){
                $delKey = $key;
            }
        }
...
//        print_r($parameters);die;
        ...
        $path = preg_replace('/\/list_(\d+).html/i','',$path);
        // $url = $path;
        if (!empty($parameters)) {

            $url = $path.'/'.$this->options['var_page'].'_'.$page.'.html';
