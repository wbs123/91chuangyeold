...
作者:noybo
邮箱:noyobo@gmail.com
博客:http://www.cnblogs.com/noyobo
------------------
*/
(function($){
	$.fn.emailMatch= function(settings){
		var defaultSettings = {
			emailTip:"",
			aEmail:["163.com","qq.com","gmail.com","yahoo.com","sohu.com","126.com","sina.com","hotmail.com"], //邮件数组
			wrapLayer:"body",
		...
		return this.each(function(){
			...
			switch(settings.position){  // 判断 列表位置
				case "bottom":
					t = elem.position().top+3;//+260
					l = elem.position().left;//+132
				break;
				case "left":
			...
			elem.focus(function(){
				...
				// 还原 输入框 提示内容
				if(!elem.val().match(/^\w(\w|_|-|\.)*@\w(\w|_|-|\.)*\.(com|edu|gov|int|mil|net|org|biz|info|pro|name|museum|coop|aero|xxx|idv|cn|hk|tw)$/))
				{
	...
	function createMailList(oWrap,sVal,suffix,aEail,nK){ // 创建 候选列表
		...
		mailList = "<iframe frameborder='0'></iframe><ul>" + mailList + "</ul>";
		$(oWrap).html(mailList);
	};
})(jQuery);